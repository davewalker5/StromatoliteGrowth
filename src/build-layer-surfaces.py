...
def spatial_coordinates(spatial_shape: tuple[int, ...]) -> dict[str, np.ndarray]:
    ...
    :param spatial_shape: Non-time dimensions of the stratigraphy archive.
    :return: Dictionary containing x_mm and, for surface grids, y_mm vectors.
    """
...
def layer_ids_from(layer_id: np.ndarray) -> np.ndarray:
    ...
    :param layer_id: Timestep layer-id raster stack from the stratigraphy NPZ.
    :return: Sorted layer IDs, excluding masked or inactive negative IDs.
    """
    # Circular models use -1 outside the active mask; those cells are not real layers.
...
def layer_step_bounds(layer_id: np.ndarray, layer_ids: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    :param layer_id: Timestep layer-id raster stack from the stratigraphy NPZ.
    :param layer_ids: Layer IDs to locate in the timestep stack.
    :return: Start-step and end-step vectors aligned with layer_ids.
    """
    ...
    for index, current_layer_id in enumerate(layer_ids):
        # Reduce each spatial raster to a single boolean: did this layer exist anywhere
        # on the active surface at this timestep?
...
def build_layer_surfaces(input_path: Path) -> dict[str, np.ndarray]:
    ...
    :param input_path: Existing stratigraphy NPZ file to convert.
    :return: Browser-ready arrays keyed by output NPZ field name.
    """
    with np.load(input_path) as archive:
        ...
        # The deposition and layer-id stacks must have matching timestep and spatial
        # dimensions so every deposited increment can be assigned to exactly one layer.
        ...
        if "active_mask" in archive.files:
            ...
            # Rectangular outputs do not need an explicit mask; every grid cell is active.
            active_mask = np.ones(spatial_shape, dtype=bool)

        if "initial_dome_height_mm" in archive.files:
            # Domed outputs store a base geometry that should be added to cumulative
            # growth for rendering total height above the original base.
        ...
        for current_layer_id in layer_ids:
            # Isolate all timestep deposition increments assigned to this layer, then
            # collapse time to get the layer's total local thickness.
            ...
            # Keep masked cells as NaN so downstream Plotly renders a circular domain
            # rather than drawing a rectangular sheet around it.
        ...
        # height_mm is the rendered geometry: cumulative growth plus any initial dome.
        ...
        output = {
            # Store coordinates as vectors; the renderer can rebuild mesh grids cheaply.
...
def main() -> None:
    ...
    # Compression keeps the browser-ready archive small without changing NumPy loading.
